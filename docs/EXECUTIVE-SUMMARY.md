...
**Date:** 2026-09-23. **Source:** [protocol-emulator ASIC competition](https://blog.janestreet.com/protocol-emulator-asic-competition/). **Top:** `tt_um_loom_gpe`. **Tiles:** 6×4. **Clock:** 50 MHz. **License:** Apache-2.0.
...
**Status.** The design is a reprogrammable pin/time ISA, not a hardwired UART/SPI/I2C trio. The 2-state-machine / 4-slot CMOS5L GDS with IHP 2-port SRAM imem (`RM_IHPSG13_2P_256x16_c2_bm_bist`) closed at 50 MHz in CI (commit `d69796a`) with Magic DRC 0, LVS clean and antenna 0; it is the current source. Protocol graphs exist for every named contest protocol. On the interpreter, all 10 registered TX/RX pairs are exhaustive codecs for bytes 0..255 against independent spec languages. USB and CAN time bits from a helper clock on pin2. k-induction covers 1-SM ISA + CRC MOV + host CSR writes, ClockedImem, and 2-SM fetch + SM0/SM1 CSR writes. UART/SPI/I2C hello pin traces match Amaranth RTL (push-pull I2C hello graphs still exist; open-drain is the I2C formal case). The cocotb wrapper test runs on the hardened netlist: SM0 encodes `Hi` as UART on pin0, the bench loops it back, SM1 decodes it, and the host pops `Hi` from the RX FIFO. Nothing has been run on an FPGA or a real peer device.

| Closed 2-SM + SRAM GDS | 54 k-induction assertions | 10 × 256 interp codecs | Gate-level UART | FPGA / real pin |
|---|---|---|---|---|
| 50 MHz, DRC 0, LVS match, +7.46 ns | ISA + FIFO/halt, not protocols | All registered pairs | `Hi` TX→RX on the netlist | 0 bring-up runs |
...
| Area: 6×4 (~0.7 mm², ~24k cell budget) | Closed GDS (2 SM, 4 slots): 5,086 std cells, 71,469 µm², plus the IHP 2P SRAM macro 57,521 µm². 14.3% of the core. | Yes — under budget |
| Full P&R + timing at a declared clock | 2-SM + SRAM GDS: setup slack slow/typ/fast +7.46 / +10.14 / +10.69 ns @ 50 MHz, worst hold +0.11 ns. Magic DRC 0, LVS clean, antenna 0, route DRC 0. 10 max-slew / 41 max-fanout / 8 max-cap at slow corner (non-blocking). Precheck passes. | Yes |
...
`emit.py` writes two engines into `src/loom_engine.v`: `loom_engine` (1 SM, 1 slot) and `loom_chip` (2 SM, 4 slots). `src/project.v` instantiates `loom_chip`, and that is what the closed GDS contains.

| | Closed GDS (`tt_submission`, CI `d69796a`) | Superseded 1-SM GDS (`f552ddc`) |
|---|---|---|
| Instantiates | `loom_chip` | `loom_engine` |
| SMs / slots | 2 SM, 4×32-word imem in IHP 2P SRAM | 1 SM, 1×32-word FF imem |
| Std cells / FFs | 5,086 / 485 + SRAM macro | 3,499 / 623 |
| Area | 71,469 µm² cells + 57,521 µm² macro (14.3% of 6×4) | 56,492 µm² (~8%) |
| DRC / LVS / 50 MHz | Magic DRC 0, LVS clean, slow setup +7.46 ns | Magic DRC 0, LVS clean, +8.99 ns |

k-induction, iverilog protocol tests, and the exhaustive ISA sweep target `GraphEngine()` / `loom_engine`; the 2-SM fetch and SM0/SM1 CSR writes have their own harness. Dual-SM at gate level is the UART round-trip in `test/test.py` (SM0 TX, SM1 RX).
...
| Gate-level cocotb | `test/test.py` on the hardened netlist | Yes — UART `Hi`: SM0 TX, pin0 loopback, SM1 RX, host pop; bench-side 8N1 decode of pin0 |
...
| 2-SM / 4-slot in k-induction | ISA harness is 1 SM; 2-SM fetch + CSR harness is separate | Partial |
...
After fabrication of the closed GDS, a host could in principle load `uart_8n1` or `spi` or `i2c_od` and bit-bang those byte-level contracts on 8 GPIO, at rates the 50 MHz divider can hit. That is an architectural claim plus simulation. It has not been demonstrated.
