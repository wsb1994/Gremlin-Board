...
**Status.** The design is a reprogrammable pin/time ISA, not a hardwired UART/SPI/I2C trio. A 1-state-machine CMOS5L GDS closed at 50 MHz with Magic DRC 0 and a unique LVS match. Current source instantiates a larger 2-SM / 4-slot chip that has been synthesised but not taken to GDS. Protocol graphs exist for every named contest protocol. On the interpreter, all 10 registered TX/RX pairs (UART hello, UART 8N1, SPI mode-0 with CS, I2C open-drain, JTAG TMS TAP, SWD line-reset, PS/2 host byte, CAN stuffed CRC-15, USB LS NRZI, Ethernet MAC framing) are exhaustive codecs for bytes 0..255 against independent spec languages. USB and CAN time bits from a helper clock on pin2; that pin is not part of those buses. The ISA is k-induction proven on the 1-SM engine (54 named assertions). UART/SPI/I2C hello pin traces also match Amaranth RTL (push-pull I2C, not the open-drain pair). Nothing has been run on an FPGA or a real peer device.
...
| Formal methods | Yosys `sat -tempinduct`, 54 named assertions, 1-SM `GraphEngine()`, default CSRs. Log: `Induction step proven: SUCCESS!` (`test/gen/formal.log`). Does not mention UART/SPI/I2C. Does not prove a protocol waveform. | Yes — ISA, not protocols |
...
| Area: 6×4 (~0.7 mm², ~24k cell budget) | Closed GDS (1 SM): 3,499 cells, 56,492 µm². Current 2-SM synth: 14,944 cells, 233,174 µm². Imem is stdcell FFs, not SRAM. | Yes — under budget |
